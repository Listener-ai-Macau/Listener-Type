...
- **買鍵盤，送訂閱。** 軟體開源免費，今天下載就能用，不強制註冊；鍵盤隨附雲端服務訂閱，開箱就是完整體驗。辨識也可以完全走本機模型，聲音去了哪裡由你決定。
...
- **六顆燈替你盯著進度。** 綠是好了，金是在聽，紫是在想，紅是有事——不用看螢幕就知道進行到哪。
- **擺得上檯面。** 透明鍵帽、金屬旋鈕、分區可調的燈光，是桌面上的一件物件，不只是一個外設。
- **按鍵出廠就有用。** KEY3 預設就是貼上，四顆鍵的單擊、雙擊、長按都能自訂；沒設定的鍵只發無害的備援鍵碼，永遠不會自己亂打字。
- **放著不用會自己睡。** 無操作自動滅燈小睡，任何按鍵一碰就醒；長時間不用自動關機，單擊旋鈕帶回來。
