...
6. 在第二遍播放前向串口 helper 发信号。产品验收用 `generated-key3` 发送 `~KEY:KEY3:SINGLE` 生成真实自定义录音键按下/释放时序；EC11 单击是开机后的自定义动作键，不再作为录音触发；`serial-toggle` 仅保留为底层 `~VREC:TOGGLE` 诊断。
...
| `trigger` | Trigger mode: `generated-key3`, `serial-toggle`, `serial-cancel`, `desktop-cancel`, or `manual-key`. A1/A2 acceptance uses `generated-key3`; A3 owns ASR accuracy gating. |
