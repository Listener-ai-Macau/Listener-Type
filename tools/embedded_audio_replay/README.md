...
真实设备 smoke 默认不要人工按物理键。优先使用串口控制命令模拟默认 EC11 单击录音键，并把录音窗口对齐到第二遍 TTS 播放：
...
| `trigger` | Trigger mode: `generated-key3`, `serial-toggle`, `serial-cancel`, `desktop-cancel`, or `manual-key`. A1/A2 acceptance uses `generated-key3`; A3 owns ASR accuracy gating. EC11 custom-click evidence belongs to the `Shift+F13` custom-key path, not the recording trigger matrix. |
