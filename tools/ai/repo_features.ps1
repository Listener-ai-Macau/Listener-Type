...
function Test-FeatureSnapshot {
    ...
    $errors += @(Test-RepoText "src-tauri/src/types.rs" 'DEFAULT_DEVICE_PLUGGED_LOW_POWER_ENABLED:\s*bool\s*=\s*true' 'plugged low-power default')
    ...
    $errors += @(Test-RepoText "src/lib/ipc.ts" 'devicePluggedLowPowerEnabled:\s*true' 'mock plugged low-power default')
    $errors += @(Test-RepoText "src/pages/settings/DeviceSection.tsx" 'pluggedBrightnessPercent:\s*snapshot\.pluggedBrightnessPercent' 'settings plugged brightness snapshot mapping')
    $errors += @(Test-RepoText "src/pages/settings/DeviceSection.tsx" 'batteryBrightnessPercent:\s*snapshot\.batteryBrightnessPercent' 'settings battery brightness snapshot mapping')
    $errors += @(Test-RepoText "src/pages/settings/DeviceSection.tsx" 'pluggedLowPowerEnabled:\s*snapshot\.pluggedLowPowerEnabled' 'settings plugged low-power snapshot mapping')
