...
//!    的 provider 私有字段更直接；Listener Type 只做渠道级开关，不维护单模型适配表。
...
use crate::polish::{
    ...
    compose_translate_prompts, http_client_builder_with_proxy, safe_str_slice, LLMError,
    ProviderProxyConfig,
};
...
impl GeminiConfig {
    pub fn new(
        ...
        Self {
            ...
            proxy_config: ProviderProxyConfig::provider_default("gemini"),
...
///
/// Listener Type 不维护 Gemini 单模型适配表；开启时不下发 thinkingConfig，关闭时
