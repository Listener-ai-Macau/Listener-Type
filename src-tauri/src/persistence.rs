...
//!
//! Storage roots:
//! - macOS:   `~/Library/Application Support/Listener Type`
//! - Windows: `%APPDATA%\Listener Type`
//! - Linux:   `$XDG_DATA_HOME/Listener Type` or `~/.local/share/Listener Type`
//!
...
/// 与 Swift `Sources/ListenerTypePersistence/DictionaryStore.swift` 同名，
...
/// times (1 manifest + N chunks) entries → tens of "Listener Type wants to use
...
/// 一样放在 Listener Type 的 models 目录下，卸载清理用户数据时可以一起删除。
#[cfg(target_os = "windows")]
...
impl CredsAsrEntry {
    fn is_empty(&self) -> bool {
        self.apiKey.as_deref().unwrap_or("").is_empty()
            ...
            && self.proxyMode.as_deref().unwrap_or("").is_empty()
            && self.proxyURL.as_deref().unwrap_or("").is_empty()
    }
}
...
fn credentials_path() -> Result<PathBuf> {
    // macOS / Linux: ~/.listener-type/credentials.json (与 Swift 同源)
    // Windows: %APPDATA%\Listener Type\credentials.json (Windows 没有标准 HOME 环境变量)
    #[cfg(target_os = "windows")]
    {
        let appdata = std::env::var("APPDATA").context("APPDATA not set")?;
...
fn load_credentials() -> CredsRoot {
    ...
    match load_keyring_credentials() {
        Ok(Some(root)) => {
            ...
            // 只会反复弹「Listener Type 想删除 X」十几次。文件 legacy（plaintext
...
fn save_credentials(root: &CredsRoot) -> Result<()> {
    ...
    let manifest = CredsChunkManifest {
        listener_type_credentials_storage: "chunked".to_string(),
        version: 1,
        generation: None,
        chunks: chunks.len(),
    };
...
fn lookup_account(root: &CredsRoot, account: CredentialAccount) -> Option<String> {
    ...
    match account {
        ...
        CredentialAccount::LlmProxyMode => llm.and_then(|e| pick(&e.proxyMode)),
        CredentialAccount::LlmProxyUrl => llm.and_then(|e| pick(&e.proxyURL)),
        ...
        CredentialAccount::AsrProxyMode => asr.and_then(|e| pick(&e.proxyMode)),
        CredentialAccount::AsrProxyUrl => asr.and_then(|e| pick(&e.proxyURL)),
    }
}
...
fn write_account(root: &mut CredsRoot, account: CredentialAccount, value: Option<String>) {
    ...
    match account {
        ...
        CredentialAccount::LlmProxyMode => {
            let entry = root.providers.llm.entry(llm_id).or_default();
            entry.proxyMode = normalized;
        }
        CredentialAccount::LlmProxyUrl => {
            let entry = root.providers.llm.entry(llm_id).or_default();
            entry.proxyURL = normalized;
        }
...
pub enum CredentialAccount {
    ...
    AsrVocabularyId,
    /// Active ASR provider's network proxy mode.
    AsrProxyMode,
    /// Active ASR provider's custom network proxy URL.
    AsrProxyUrl,
}
...
impl CredentialAccount {
    ...
    pub fn keyring_account(&self) -> &'static str {
        match self {
            ...
            CredentialAccount::LlmProxyMode => "llm.proxy_mode",
            CredentialAccount::LlmProxyUrl => "llm.proxy_url",
            ...
            CredentialAccount::AsrProxyMode => "asr.proxy_mode",
            CredentialAccount::AsrProxyUrl => "asr.proxy_url",
        }
    }
...
impl CredentialsVault {
    ...
    pub const SERVICE_NAME: &'static str = "com.listener.type";
...
mod tests {
    ...
    fn legacy_streaming_insert_false_is_migrated_and_marker_is_persisted() {
        let tmp: PathBuf =
            std::env::temp_dir().join(format!("listener-type-prefs-test-{}", uuid::Uuid::new_v4()));
    ...
    fn vocab_presets_roundtrip_json_file() {
        let tmp: PathBuf =
            std::env::temp_dir().join(format!("listener-type-test-{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&tmp).expect("create temp dir");
