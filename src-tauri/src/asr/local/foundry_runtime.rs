...
mod imp {
    ...
    impl FoundryLocalRuntime {
        ...
        fn manager_config(&self) -> FoundryLocalConfig {
            // Listener Type owns Windows App Runtime installation; keep the SDK bootstrapper non-interactive.
            let mut config =
                FoundryLocalConfig::new("listener-type").additional_setting("Bootstrap", "false");
    ...
    mod lifecycle_tests {
        ...
        fn runtime_finds_downloaded_foundry_native_runtime_dir() {
            let root = std::env::temp_dir().join(format!(
                "listener-type-foundry-native-test-{}",
                uuid::Uuid::new_v4()
            ));
