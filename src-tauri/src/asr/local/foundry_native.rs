        ...
        F: Fn(&str, f64),
    {
        ...
        if !wait_for_windows_app_runtime_ready().await {
            ...
            if install_capture.success() {
                log::warn!(
                    "[foundry-asr] Windows App Runtime installer exited successfully, but the package probe did not confirm the complete current-user package set. Continuing with the Foundry SDK bootstrapper disabled; if Foundry fails later, use the probe diagnostics in listener-type.log. {}",
            ...
            anyhow::bail!(
                "Windows App Runtime 1.8 is still unavailable after automatic installation. Listener Type stopped before starting Foundry Local to avoid the Windows bootstrapper dialog. Installer exit code: {}. {} See listener-type.log for installer output, package list, OS build, and elevation diagnostics.",
...
"#
    ...
    mod tests {
        ...
        fn windows_app_runtime_detection_requires_complete_package_set() {
            ...
            assert!(script.contains(r"Microsoft\.WindowsAppRuntime\.1\.8"));
            ...
            assert!(script.contains(r"Main\.1\.8"));
        ...
        fn extract_native_libraries_only_keeps_target_rid_dlls() {
            let root = std::env::temp_dir().join(format!(
                "listener-type-foundry-native-zip-test-{}",
                uuid::Uuid::new_v4()
            ));
