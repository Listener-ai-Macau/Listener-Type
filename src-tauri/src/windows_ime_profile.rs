pub const LISTENER_TYPE_TSF_LANG_ID: u16 = 0x0804;
pub const LISTENER_TYPE_TEXT_SERVICE_CLSID_BRACED: &str = "{E6D16C6C-2975-4A5C-BBBB-67A3C9966767}";
pub const LISTENER_TYPE_PROFILE_GUID_BRACED: &str = "{19F96D43-A5EB-46C9-8A73-9FCA5A0630C8}";
...
pub fn restore_decision(
    ...
    listener_type_profile_is_current: bool,
    listener_type_activation_failed: bool,
) -> ProfileRestoreDecision {
    if saved.is_some() && (listener_type_profile_is_current || listener_type_activation_failed) {
...
impl WindowsImeProfileManager {
    ...
    pub fn activate_listener_type_profile(&self) -> WindowsImeProfileResult<()> {
        windows_impl::activate_listener_type_profile()
    }
...
mod windows_impl {
    ...
    const LISTENER_TYPE_COM_INPROC_KEY: &str =
        r"Software\Classes\CLSID\{E6D16C6C-2975-4A5C-BBBB-67A3C9966767}\InprocServer32";
    const LISTENER_TYPE_TSF_PROFILE_KEY: &str = r"Software\Microsoft\CTF\TIP\{E6D16C6C-2975-4A5C-BBBB-67A3C9966767}\LanguageProfile\0x00000804\{19F96D43-A5EB-46C9-8A73-9FCA5A0630C8}";
    ...
    pub fn activate_listener_type_profile() -> WindowsImeProfileResult<()> {
    ...
    pub fn is_listener_type_profile_active() -> WindowsImeProfileResult<bool> {
        ...
        Ok(matches!(snapshot.kind(), ImeProfileKind::TextService)
            && snapshot.lang_id() == LISTENER_TYPE_TSF_LANG_ID
            && snapshot.clsid().map(normalize_guid_string).as_deref()
                == Some(LISTENER_TYPE_TEXT_SERVICE_CLSID_BRACED)
            && snapshot
                .profile_guid()
                .map(normalize_guid_string)
                .as_deref()
                == Some(LISTENER_TYPE_PROFILE_GUID_BRACED))
    }
    ...
    pub fn get_windows_ime_status() -> WindowsImeStatus {
        match inspect_windows_ime_registration() {
            RegistrationInspection::Installed { dll_path } => WindowsImeStatus {
                ...
                message: "Listener Type TSF IME registration is present".to_string(),
                dll_path: Some(dll_path),
            },
            RegistrationInspection::NotInstalled => WindowsImeStatus {
                state: WindowsImeInstallState::NotInstalled,
                using_tsf_backend: false,
                message: "Listener Type TSF IME registration was not found".to_string(),
                dll_path: None,
            },
    ...
    fn inspect_windows_ime_registration() -> RegistrationInspection {
        ...
        let com_key =
            hklm.open_subkey_with_flags(LISTENER_TYPE_COM_INPROC_KEY, KEY_READ | KEY_WOW64_64KEY);
        let tip_key_exists = hklm
            .open_subkey_with_flags(LISTENER_TYPE_TSF_PROFILE_KEY, KEY_READ | KEY_WOW64_64KEY)
            .is_ok();
        let keyboard_category_exists = hklm
            .open_subkey_with_flags(
                LISTENER_TYPE_TSF_KEYBOARD_CATEGORY_KEY,
                KEY_READ | KEY_WOW64_64KEY,
            )
            .is_ok();
        let immersive_category_exists = hklm
            .open_subkey_with_flags(
                LISTENER_TYPE_TSF_IMMERSIVE_CATEGORY_KEY,
                KEY_READ | KEY_WOW64_64KEY,
            )
            .is_ok();
        let systray_category_exists = hklm
            .open_subkey_with_flags(
                LISTENER_TYPE_TSF_SYSTRAY_CATEGORY_KEY,
                KEY_READ | KEY_WOW64_64KEY,
            )
            .is_ok();
        ...
        let com_key = match com_key {
            ...
            Err(_) => {
                return RegistrationInspection::Broken {
                    dll_path: None,
                    reason: "Listener Type COM registration is missing".to_string(),
                };
            }
        };
        ...
        let dll_path: String = match com_key.get_value::<String, _>("") {
            ...
            _ => {
                return RegistrationInspection::Broken {
                    dll_path: None,
                    reason: "Listener Type COM DLL path is missing".to_string(),
                };
            }
        };
        ...
        if !Path::new(&dll_path).is_file() {
            return RegistrationInspection::Broken {
                dll_path: Some(dll_path),
                reason: "Listener Type COM DLL path does not exist".to_string(),
            };
        }
        ...
        if !Path::new(&x86_dll_path).is_file() {
            return RegistrationInspection::Broken {
                ...
                reason: "Listener Type 32-bit COM DLL path does not exist".to_string(),
            };
        }
        ...
        if !tip_key_exists {
            return RegistrationInspection::Broken {
                dll_path: Some(dll_path),
                reason: "Listener Type TSF language profile registration is missing".to_string(),
            };
        }
        ...
        if !keyboard_category_exists {
            return RegistrationInspection::Broken {
                dll_path: Some(dll_path),
                reason: "Listener Type TSF keyboard category registration is missing".to_string(),
            };
        }
    ...
    fn read_com_dll_path(hklm: &RegKey, flags: u32, label: &str) -> Result<String, String> {
        let com_key = hklm
            .open_subkey_with_flags(LISTENER_TYPE_COM_INPROC_KEY, flags)
            .map_err(|_| format!("Listener Type {label} COM registration is missing"))?;
        match com_key.get_value::<String, _>("") {
            Ok(value) if !value.trim().is_empty() => Ok(value),
            _ => Err(format!("Listener Type {label} COM DLL path is missing")),
        }
    }
...
mod tests {
    ...
    #[test]
    fn restore_is_required_when_listener_type_is_active_and_snapshot_exists() {
        assert_eq!(
    ...
    #[test]
    fn restore_is_skipped_when_user_already_changed_away_from_listener_type() {
        assert_eq!(
...
mod windows_tests {
    ...
    #[test]
    fn listener_type_profile_identifiers_are_fixed() {
        assert_eq!(LISTENER_TYPE_TSF_LANG_ID, 0x0804);
        assert_eq!(
            LISTENER_TYPE_TEXT_SERVICE_CLSID_BRACED,
            "{E6D16C6C-2975-4A5C-BBBB-67A3C9966767}"
        );
        assert_eq!(
            LISTENER_TYPE_PROFILE_GUID_BRACED,
            "{19F96D43-A5EB-46C9-8A73-9FCA5A0630C8}"
        );
    }
    ...
    fn guid_snapshot_strings_are_canonical_and_parseable() {
        let guid = windows::core::GUID::from_u128(0xe6d16c6c_2975_4a5c_bbbb_67a3c9966767);
        let formatted = windows_impl::guid_to_braced_string(guid);

        assert_eq!(formatted, "{E6D16C6C-2975-4A5C-BBBB-67A3C9966767}");
