...
//!   `ListenerTypeHotkey/HotkeyMonitor.swift` 同源。**不能用 `rdev`**：rdev 在每个
...
/// `listener-type --toggle-dictation`」的引导文案。
...
mod platform {
    ...
    pub fn start_adapter(
        ...
        let listener = start_listener_thread(
            binding,
            tx,
            "listener-type-hotkey-mac-event-tap",
            "hotkey hook 启动超时",
            run_listen_loop,
        )?;
...
mod platform {
    ...
    const ACCEPT_INJECTED_ENV: &str = "LISTENER_TYPE_ACCEPT_SYNTHETIC_HOTKEY_EVENTS";
    ...
    pub fn start_adapter(
        ...
        let listener = start_listener_thread(
            binding,
            tx,
            "listener-type-hotkey-win-ll-hook",
...
mod platform {
    ...
    /// 不再把 Wayland 当成"安装失败"。
    ///
    /// 用户实际的触发路径变成：桌面环境快捷键 → `listener-type --toggle-dictation`
        ...
        if super::is_wayland_session() {
            log::info!(
                "[hotkey] Wayland session detected; rdev listener skipped — \
                 use desktop shortcut → `listener-type --toggle-dictation` instead (issue #420)"
            );
