...
function normalizeUserPreferences(prefs: UserPreferences): UserPreferences {
  ...
  const deviceKnobLedBrightnessPercent = clampNumber(
    prefs.deviceKnobLedBrightnessPercent,
    DEFAULT_DEVICE_LED_ZONE_BRIGHTNESS_PERCENT,
    0,
    100,
  );
  const deviceEdgeLedBrightnessPercent = clampNumber(
    prefs.deviceEdgeLedBrightnessPercent,
    DEFAULT_DEVICE_LED_ZONE_BRIGHTNESS_PERCENT,
    0,
    100,
  );
  let deviceStatusLedBrightnessPercent = clampNumber(
    prefs.deviceStatusLedBrightnessPercent,
    DEFAULT_DEVICE_STATUS_LED_BRIGHTNESS_PERCENT,
    0,
    100,
  );
  if (
    ...
    prefs.deviceStatusLedBrightnessPercent === DEFAULT_DEVICE_LED_ZONE_BRIGHTNESS_PERCENT &&
  ...
  return {
    ...
    deviceCustomKeys: {
      ...
      knob: normalizeDeviceCustomKeyMapping(prefs.deviceCustomKeys?.knob, fallbackDeviceKeys.knob),
    },
    deviceCustomKeyDoubleClicks: {
      ...
      knob: normalizeDeviceCustomKeyMapping(prefs.deviceCustomKeyDoubleClicks?.knob, fallbackDisabledDeviceKeys.knob),
    },
    deviceCustomKeyLongPresses: {
      ...
      knob: normalizeDeviceCustomKeyMapping(prefs.deviceCustomKeyLongPresses?.knob, fallbackDisabledDeviceKeys.knob),
    },
    ...
    deviceStatusLedBrightnessPercent,
    deviceKeyLedBrightnessPercent,
...
function composeMockStylePackRuntimeDiagnostics(stylePack: StylePack): StylePackRuntimeDiagnostics {
  ...
  const contextPremise = mockSettings.workingLanguages.length
    ...
    : '';
  const hotwordLines = [`GitHub`, `Listener Type`];
...
// 把 `listener-type --toggle-dictation` 绑到桌面环境快捷键。浏览器 / 非 Tauri 环境下永远 false。
...
/**
 * 让用户选 save 路径并把当前会话日志（listener-type.log）复制过去。
