...
- **买键盘，送订阅。** 软件开源免费，下载就能用，不用注册；键盘会附赠云服务订阅。不想用云也行，识别可以完全跑在本机，声音去哪你自己定。
- **说完就是成品。** 它会顺手去掉「呃」「那个」，补上标点，把一段唠叨理成能直接发出去的文字。
- **手不用回到键盘。** 说一声「开始录音」就开工，再按一下旋钮就停，人不用凑在电脑前。
- **只认你的声音。** 录三段声纹之后，旁边同事聊什么不会进你的稿子。
- **选中就能问。** 看到一段不懂的文字，选中它，直接开口问。
- **六颗灯各管一件事。** 绿是好了，金是在听，紫是在想，红是有事——不用看屏幕。
- **放在桌上不难看。** 透明键帽，金属旋钮，每个灯区的亮度都能单独调。
- **按键出厂就有用。** KEY3 默认就是粘贴，四个键的单击、双击、长按都能改；没设置的键不会乱打字，只发系统用不上的兜底键码。
- **没人理就自己去睡。** 放着不管，一分钟左右灭灯打盹，碰任意键就醒；十来分钟没动静会自动关机，按一下旋钮带回来。

系统自带的语音输入把字打出来就不管了。Listener 多做半步：整理、翻译，还记你的词库——数据交给谁，由你选的 Provider 说了算。
