...
**记需求、任务、prompt。** Structured 按主题和目标整理成条目。写代码注释、或者要留原话的时候用 Raw,它尽量不动你的词。风格本身就是本地文件:内置的可以复制出来改,也能导出成 ZIP 分享。
...
没有键盘软件照常用,键盘只是把录音键放到手指底下。配对乱了也不用开发工具:设置 → 关于 → 设备恢复。固件也开源:[Listener-Firmware](https://github.com/Listener-ai-Macau/Listener-Firmware)。

## 你的数据和你的 Key

识别可以走火山引擎流式接口、任意 OpenAI 兼容端点、Apple Speech,或完全在本机;润色接 Ark、DeepSeek,或任意 Anthropic / OpenAI 兼容端点。Key 存在系统凭据库,软件不自带任何 Key;每个服务商可以单独选直连、系统代理或自定义代理。历史、词库、风格、设置都在本机——整条链路不依赖 Listener 的服务器。
