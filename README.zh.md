...
<p align="center">
  <a href="https://github.com/Listener-ai-Macau/Listener-Type/actions/workflows/ci.yml"><img alt="仓库检查" src="https://github.com/Listener-ai-Macau/Listener-Type/actions/workflows/ci.yml/badge.svg" /></a>
...
| 语音键盘 | 配对与健康状态、四颗自定义键、按压/旋转旋钮、灯光亮度、低功耗时间、电量、OTA 和设备恢复 |
...
在设置 → 设备中配对。单击旋钮开始或停止，双击重置配对，长按关机，旋转可调音量或亮度。KEY1–KEY4 的单击、双击、长按都能配置动作。PWR、BLE、REC、AI、OK、WARN 分别显示电源、连接、收音、处理、成功和错误状态。
...
完整桌面构建还需要 `third_party/denzic-platform` 子模块。贡献前请阅读 [CONTRIBUTING.md](CONTRIBUTING.md)。当前仓库没有 `LICENSE` 文件，因此能看到源码不代表自动获得再分发或修改授权。
