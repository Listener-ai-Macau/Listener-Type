...
def focus_notepad_window():
    fixture = Path(tempfile.gettempdir()) / f"listener-type-lifecycle-{uuid.uuid4().hex}.txt"
...
def main():
    ...
    parser.add_argument("--phrase", default="listener-type terminal regression success")
    ...
    if args.injected_transcript_text.strip():
        debug_transcript_path = str(Path(tempfile.gettempdir()) / "listener-type-debug-transcript-e2e.txt")
...
"""
    ...
    clipboard_sentinel = f"LISTENER_TYPE_OLD_CLIPBOARD_SENTINEL_{uuid.uuid4().hex}"
