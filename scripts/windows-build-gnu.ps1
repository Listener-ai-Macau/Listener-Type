param(
  [string]$MirrorRoot = "$env:TEMP\listener-type-windows-gnu",
...
try {
  ...
  Copy-Item -LiteralPath (Join-Path $releaseRoot "listener-type.exe") -Destination (Join-Path $artifactDevRoot "listener-type.exe") -Force
  Copy-Item -LiteralPath (Resolve-WebView2Loader) -Destination (Join-Path $artifactDevRoot "WebView2Loader.dll") -Force

  # Build ListenerTypeIme.dll (x64 + x86) 并导出绝对路径 env var——
  # nsis/listener-type-ime-hooks.nsh 的 PREINSTALL 用 File "$%LISTENER_TYPE_IME_DLL_X64%" / _X86
  # 在 makensis 编译时把 dll 嵌入 NSIS 包；wix/listener-type-ime.wxs 也用同一对 env 解析
  ...
  foreach ($t in @(
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "ListenerTypeIme $($t.Platform) build failed with exit $LASTEXITCODE"
    }
    $dll = (Resolve-Path (Join-Path $out 'ListenerTypeIme.dll')).Path
    if (-not (Test-Path $dll)) {
      throw "ListenerTypeIme.dll not produced at $dll"
    }
...
Remove-Item -LiteralPath (Join-Path $artifactReleaseRoot "listener-type.exe") -Force -ErrorAction SilentlyContinue
...
if ($usedMirror -and (-not $KeepMirror)) {
  ...
  if ($resolvedMirror.StartsWith($resolvedTemp, [System.StringComparison]::OrdinalIgnoreCase) -and
      ((Split-Path $resolvedMirror -Leaf) -eq "listener-type-windows-gnu")) {
...
Write-Host "$ArtifactsRoot\dev\listener-type.exe"
