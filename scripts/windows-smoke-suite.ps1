param(
  [string]$ExePath = "",
  [string]$Phrase = "Listener Type Windows regression suite phrase. Listener Type Windows regression suite phrase.",
...
Write-Host "Listener Type Windows smoke suite"
...
try {
  ...
  if (-not (Test-Path $ExePath)) {
    throw "Listener Type executable not found: $ExePath. Run with -Build or run scripts/windows-build-gnu.ps1 first."
  }
