...
if (Show-ListenerTypeWindow $running) {
  Write-Host "Listener Type is already running; brought window to foreground. pid=$($running.Id)"
  exit 0
}

if (-not (Test-Path $ExePath)) {
  throw "Listener Type executable not found: $ExePath. Run scripts/windows-build-gnu.ps1 first."
}
...
while ((Get-Date) -lt $deadline) {
  ...
  if (Show-ListenerTypeWindow $current) {
    Write-Host "Listener Type started and brought to foreground. pid=$($current.Id)"
    exit 0
  }
}

throw "Listener Type started but no main window was visible within 10 seconds. pid=$($process.Id)"
