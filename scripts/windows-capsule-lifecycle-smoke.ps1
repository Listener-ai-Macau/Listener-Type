...
if ([string]::IsNullOrWhiteSpace($ExePath)) {
  ...
  $ExePath = Join-Path $appRoot "src-tauri\target\debug\listener-type.exe"
}
...
public static class ListenerTypeCapsuleProbe {
...
function Send-KeyEdge([byte]$Vk, [bool]$KeyUp) {
  $flags = [ListenerTypeCapsuleProbe]::KEYEVENTF_EXTENDEDKEY
  if ($KeyUp) {
    $flags = $flags -bor [ListenerTypeCapsuleProbe]::KEYEVENTF_KEYUP
  }
  [ListenerTypeCapsuleProbe]::keybd_event($Vk, 0x1D, $flags, [UIntPtr]::Zero)
}

function Get-CapsuleWindowState() {
  $hwnd = [ListenerTypeCapsuleProbe]::FindWindowW($null, "Listener Type Capsule")
  ...
  return [pscustomobject]@{
    Exists = $true
    Visible = [ListenerTypeCapsuleProbe]::IsWindowVisible($hwnd)
...
$env:LISTENER_TYPE_HOTKEY_INJECTION_DRY_RUN = "1"
...
try {
  ...
  Remove-Item Env:LISTENER_TYPE_HOTKEY_INJECTION_DRY_RUN -ErrorAction SilentlyContinue
  Get-Process listener-type -ErrorAction SilentlyContinue | Stop-Process -Force
}
