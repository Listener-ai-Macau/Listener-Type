...
public static class ListenerTypeCapsuleWatch {
...
function Get-CapsuleState {
  $hwnd = [ListenerTypeCapsuleWatch]::FindWindowW($null, "Listener Type Capsule")
  if ($hwnd -eq [IntPtr]::Zero) {
    return "missing"
  }
  if ([ListenerTypeCapsuleWatch]::IsWindowVisible($hwnd)) {
    return "visible"
  }
  return "hidden"
}
