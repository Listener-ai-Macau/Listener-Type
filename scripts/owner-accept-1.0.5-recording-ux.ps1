...
<#
.SYNOPSIS
  Listener 1.0.5 录音体验人工验收：分步弹窗 + 写结果 JSON。
.DESCRIPTION
...
if (-not $EvidenceDir) {
  ...
  $EvidenceDir = Join-Path $PSScriptRoot "..\docs\release\evidence\1.0.5-recording-ux-$stamp"
}
...
$steps = @(
  @{
    Id = "00-ready"; Progress = "0/4"; Title = "Listener 1.0.5 录音验收"
    Scope = "安装身份已是 Program Files 1.0.5；请确认板子已连上 Type、可唤醒。"
  ...
  @{
    Id = "01-short-snappy"; Progress = "1/4"; Title = "短句跟手（默认 1s）"
    Scope = "默认长文关；标准听写"
  ...
  @{
    Id = "02-no-mid-cut"; Progress = "2/4"; Title = "中途不掐 + 句末耐停"
  ...
  @{
    Id = "03-long-form"; Progress = "3/4"; Title = "长文模式（可选）"
    Scope = "设置 → 录音自动化 / 录音：打开「长文听写模式」"
    Message = "若方便：`n1. 打开「长文听写模式」`n2. 说一段中间有短停的口述`n3. 体感是否明显比 1s 少被掐`n4. 测完请关掉长文，恢复默认`n`n不方便可点「跳过」。"
    Expected = "长文开：短停更耐；默认关后仍 1s 跟手。"
    Spoken = ""
  },
  @{
    Id = "04-other-speaker"; Progress = "4/4"; Title = "别人说话不拖（可选）"
