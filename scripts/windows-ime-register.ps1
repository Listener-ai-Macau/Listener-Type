...
function Get-DllPath {
  ...
  return Join-Path $stagingRoot "$folder\$Configuration\ListenerTypeIme.dll"
}
...
if (-not (Test-IsAdministrator)) {
  throw "Registering the Listener Type TSF IME requires an elevated Administrator PowerShell."
}
...
foreach ($platform in @("x64", "Win32")) {
  ...
  Write-Host "[ok] Listener Type TSF IME registered ($platform)"
}
