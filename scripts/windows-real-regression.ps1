...
try {
  ...
  if (-not $live) {
    throw "Listener Type exited during startup."
  }
  if (-not $live.Responding) {
    throw "Listener Type process is not responding."
  }
  ...
  if ($PhysicalHotkey) {
    ...
    Write-Host "Press the configured physical Listener Type hotkey now. Synthetic SendInput is not accepted for this gate."
  ...
  if ($MicrophonePrivacy) {
    ...
    Write-Host "Toggle microphone privacy off, return to Listener Type Settings -> Permissions, confirm it no longer reports granted, then toggle it back on and rerun this script."
  }
} finally {
