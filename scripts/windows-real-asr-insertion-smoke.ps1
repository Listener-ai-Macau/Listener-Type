param(
  ...
  [string]$Phrase = "Listener Type Windows real regression",
...
if ([string]::IsNullOrWhiteSpace($ExePath)) {
  ...
  $ExePath = Join-Path $appRoot ".artifacts\windows-gnu\dev\listener-type.exe"
}
...
public static class ListenerTypeRegressionWin32 {
...
function Get-ListenerTypeCredentialStatus {
  $path = Join-Path $env:APPDATA "Listener Type\credentials.json"
...
function Ensure-ListenerTypeCredentialNative {
  if ("ListenerTypeCredentialNative" -as [type]) {
    return
  }
...
public static class ListenerTypeCredentialNative {
  [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
  public static extern bool CredRead(string target, UInt32 type, UInt32 reservedFlag, out IntPtr credentialPtr);

  [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
  public static extern bool CredWrite(ref ListenerTypeCredentialNativeCredential credential, UInt32 flags);
...
public struct ListenerTypeCredentialNativeCredential {
  public UInt32 Flags;
  public UInt32 Type;
...
function Get-ListenerTypeCredentialTarget($Account) {
  return "$Account.com.listener.type"
}

function Get-ListenerTypeKeyringPassword($Account) {
  Ensure-ListenerTypeCredentialNative
  $target = Get-ListenerTypeCredentialTarget $Account
  $ptr = [IntPtr]::Zero
  $ok = [ListenerTypeCredentialNative]::CredRead($target, 1, 0, [ref]$ptr)
  if (-not $ok) {
  ...
  try {
    $credential = [Runtime.InteropServices.Marshal]::PtrToStructure($ptr, [type][ListenerTypeCredentialNativeCredential])
    ...
    [ListenerTypeCredentialNative]::CredFree($ptr)
  }
}

function Set-ListenerTypeKeyringPassword($Account, $Password) {
  Ensure-ListenerTypeCredentialNative
  $target = Get-ListenerTypeCredentialTarget $Account
  ...
  try {
    $credential = [ListenerTypeCredentialNativeCredential]::new()
    $credential.Flags = 0
    $credential.Type = 1
    ...
    $ok = [ListenerTypeCredentialNative]::CredWrite([ref]$credential, 0)
    if (-not $ok) {
      $errorCode = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
...
function Remove-ListenerTypeKeyringPassword($Account) {
  Ensure-ListenerTypeCredentialNative
  $target = Get-ListenerTypeCredentialTarget $Account
  $ok = [ListenerTypeCredentialNative]::CredDelete($target, 1, 0)
  if (-not $ok) {
    $errorCode = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
...
function Split-ListenerTypeCredentialJson($Json) {
  $chunks = @()
...
function Get-ListenerTypeVaultCredentials {
  $manifestText = Get-ListenerTypeKeyringPassword "credentials.v1"
  if ([string]::IsNullOrWhiteSpace($manifestText)) {
    return $null
  }
  $manifest = $manifestText | ConvertFrom-Json
  if ($manifest.listener_type_credentials_storage -ne "chunked" -or $manifest.version -ne 1) {
    throw "Unsupported Listener Type credential vault manifest."
  }
  ...
  for ($i = 0; $i -lt [int]$manifest.chunks; $i++) {
    ...
    $chunk = Get-ListenerTypeKeyringPassword $chunkAccount
    if ($null -eq $chunk) {
      throw "Missing Listener Type credential vault chunk $i."
    }
    $json += $chunk
  }
  return $json
...
function Set-ListenerTypeVaultCredentials($Json, $PreviousManifestJson) {
  $previousManifest = $null
  if (-not [string]::IsNullOrWhiteSpace($PreviousManifestJson)) {
    $previousManifest = $PreviousManifestJson | ConvertFrom-Json
  }

  $chunks = Split-ListenerTypeCredentialJson $Json
  for ($i = 0; $i -lt $chunks.Count; $i++) {
    Set-ListenerTypeKeyringPassword "credentials.v1.chunk.$i" $chunks[$i]
  }

  $manifest = [pscustomobject]@{
    listener_type_credentials_storage = "chunked"
    version = 1
    chunks = $chunks.Count
  }
  Set-ListenerTypeKeyringPassword "credentials.v1" ($manifest | ConvertTo-Json -Compress)
  ...
  if ($null -ne $previousManifest -and $null -ne $previousManifest.PSObject.Properties["chunks"]) {
    if ($null -ne $previousManifest.PSObject.Properties["generation"] -and -not [string]::IsNullOrWhiteSpace($previousManifest.generation)) {
      for ($i = 0; $i -lt [int]$previousManifest.chunks; $i++) {
        Remove-ListenerTypeKeyringPassword "credentials.v1.chunk.$($previousManifest.generation).$i"
      }
    } else {
      for ($i = $chunks.Count; $i -lt [int]$previousManifest.chunks; $i++) {
        Remove-ListenerTypeKeyringPassword "credentials.v1.chunk.$i"
      }
    }
  }
}
...
function Restore-ActiveAsrCredential($Snapshot, $Path) {
  ...
  if ($Snapshot.HadVault) {
    ...
    $chunks = Split-ListenerTypeCredentialJson $Snapshot.VaultJson
    ...
    for ($i = 0; $i -lt $chunks.Count; $i++) {
      ...
      Set-ListenerTypeKeyringPassword $account $chunks[$i]
    }
    Set-ListenerTypeKeyringPassword "credentials.v1" $Snapshot.VaultManifestJson
    ...
    Remove-ListenerTypeKeyringPassword "credentials.v1"
    for ($i = 0; $i -lt $Snapshot.WrittenVaultChunks; $i++) {
      Remove-ListenerTypeKeyringPassword "credentials.v1.chunk.$i"
    }
  }
...
function Set-ActiveAsrCredential($Path) {
  ...
  $previousManifest = Get-ListenerTypeKeyringPassword "credentials.v1"
  $previousVault = Get-ListenerTypeVaultCredentials
  ...
  $chunks = Split-ListenerTypeCredentialJson $json
  Set-ListenerTypeVaultCredentials $json $previousManifest
...
function Send-KeyEdge($Vk, $KeyUp, $Extended = $true) {
  $flags = 0
  if ($Extended) {
    $flags = $flags -bor [ListenerTypeRegressionWin32]::KEYEVENTF_EXTENDEDKEY
  }
  if ($KeyUp) {
    $flags = $flags -bor [ListenerTypeRegressionWin32]::KEYEVENTF_KEYUP
  }
  $scanCode = if ($Vk -eq 0xA3 -or $Vk -eq 0xA2) { 0x1D } else { 0 }
  [ListenerTypeRegressionWin32]::keybd_event([byte]$Vk, [byte]$scanCode, $flags, [UIntPtr]::Zero)
}
...
function Focus-Window($Process) {
  ...
  [ListenerTypeRegressionWin32]::ShowWindow($Process.MainWindowHandle, 9) | Out-Null
  [ListenerTypeRegressionWin32]::SetForegroundWindow($Process.MainWindowHandle) | Out-Null
  Start-Sleep -Milliseconds 500
  return $true
}
...
function New-BrowserInputFixture {
  $path = Join-Path $env:TEMP "listener-type-browser-input-fixture.html"
  $html = @"
<!doctype html>
<html>
<head>
  <meta charset="utf-8">
  <title>Listener Type Browser Input Fixture</title>
  <style>
...
function New-Win32EditHost {
  $sourcePath = Join-Path $env:TEMP "ListenerTypeWin32EditHost.cs"
  $exePath = Join-Path $env:TEMP "ListenerTypeWin32EditHost.exe"
  $source = @"
using System;
using System.Windows.Forms;

public static class ListenerTypeWin32EditHost {
  [STAThread]
  public static void Main() {
    Application.EnableVisualStyles();
    Application.SetCompatibleTextRenderingDefault(false);
    var form = new Form();
    form.Text = "Listener Type Win32 Edit Host";
    form.Width = 820;
    form.Height = 320;
    var box = new TextBox();
    box.Multiline = true;
...
function Start-InputTarget($TargetName) {
  ...
  if ($TargetName -eq "notepad") {
    $fixture = Join-Path $env:TEMP "listener-type-notepad-input-fixture.txt"
    Write-TextUtf8 $fixture ""
    $process = Start-Process notepad.exe -ArgumentList $fixture -PassThru
    Start-Sleep -Seconds 2
    $title = "listener-type-notepad-input-fixture.txt - Notepad"
    $activateScript = @"
...
raise SystemExit(1)
"@
    $activatePath = Join-Path $env:TEMP "listener-type-activate-notepad.py"
...
raise SystemExit(1)
"@
    $activatePath = Join-Path $env:TEMP "listener-type-activate-target.py"
    Write-TextUtf8 $activatePath $activateScript
    try {
      python $activatePath $title | Out-Null
    } finally {
...
raise SystemExit(1)
"@
    $handlePath = Join-Path $env:TEMP "listener-type-terminal-handle.py"
  ...
  if ($TargetName -eq "win32edit") {
    ...
    $process = Wait-ProcessWindow "ListenerTypeWin32EditHost" $startedAt 15
  ...
  $profilePath = Join-Path $env:TEMP "listener-type-browser-smoke-profile"
...
raise SystemExit(1)
"@
    $readbackPath = Join-Path $env:TEMP "listener-type-notepad-readback.py"
    $outputPath = Join-Path $env:TEMP "listener-type-notepad-readback.txt"
...
raise SystemExit(1)
"@
    $readbackPath = Join-Path $env:TEMP "listener-type-terminal-readback.py"
    $outputPath = Join-Path $env:TEMP "listener-type-terminal-readback.txt"
    Write-TextUtf8 $readbackPath $readbackScript
    try {
      Remove-Item -LiteralPath $outputPath -Force -ErrorAction SilentlyContinue
...
$credentialStatus = Get-ListenerTypeCredentialStatus
...
$credentialsPath = Join-Path $env:APPDATA "Listener Type\credentials.json"
$inputTarget = $null
$listener-type = $null
...
try {
  ...
  $clipboardSentinel = "LISTENER_TYPE_OLD_CLIPBOARD_SENTINEL_$(Get-Date -Format 'yyyyMMddHHmmssfff')"
  Restore-ClipboardValue $clipboardSentinel
  if (-not [string]::IsNullOrWhiteSpace($InjectedTranscriptText)) {
    $debugTranscriptPath = Join-Path $env:TEMP "listener-type-debug-transcript.txt"
  ...
  if ($DebugHotkeyEvents) {
    $env:LISTENER_TYPE_DEBUG_HOTKEY_EVENTS = "1"
  }
  if ($debugTranscriptPath) {
    $env:LISTENER_TYPE_DEBUG_TRANSCRIPT_FILE = $debugTranscriptPath
  }
  try {
    $listener-type = Start-Process -FilePath $ExePath -WorkingDirectory (Split-Path $ExePath -Parent) -PassThru
  } finally {
    Remove-Item Env:LISTENER_TYPE_SHOW_MAIN_ON_START -ErrorAction SilentlyContinue
    Remove-Item Env:LISTENER_TYPE_ACCEPT_SYNTHETIC_HOTKEY_EVENTS -ErrorAction SilentlyContinue
    Remove-Item Env:LISTENER_TYPE_DEBUG_HOTKEY_EVENTS -ErrorAction SilentlyContinue
    Remove-Item Env:LISTENER_TYPE_DEBUG_TRANSCRIPT_FILE -ErrorAction SilentlyContinue
  }
  ...
  if (-not (Wait-LogPattern $logPath "\[coord\] session started" 30)) {
    throw "Listener Type recording session did not start."
  }
  ...
  if (Test-Path $logPath) {
    ...
    if ($logText -match $forbiddenNativeDictationPattern) {
      throw "Listener Type log contains a native Windows dictation route marker; this smoke must use the Listener Type pipeline."
    }
  }
} finally {
  Release-Hotkey
