...
if ([string]::IsNullOrWhiteSpace($ExePath)) {
  ...
  $ExePath = Join-Path $appRoot "src-tauri\target\x86_64-pc-windows-gnu\release\listener-type.exe"
}
...
public static class ListenerTypeInput {
...
function Send-KeyEdge($Vk, $KeyUp) {
  $flags = [ListenerTypeInput]::KEYEVENTF_EXTENDEDKEY
  if ($KeyUp) {
    $flags = $flags -bor [ListenerTypeInput]::KEYEVENTF_KEYUP
  }
  [ListenerTypeInput]::keybd_event([byte]$Vk, 0x1D, $flags, [UIntPtr]::Zero)
}
...
function Focus-Window($Process) {
  ...
  [ListenerTypeInput]::ShowWindow($Process.MainWindowHandle, 9) | Out-Null
  [ListenerTypeInput]::SetForegroundWindow($Process.MainWindowHandle) | Out-Null
