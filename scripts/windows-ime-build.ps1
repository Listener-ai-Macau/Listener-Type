...
$solution = Join-Path $appRoot "windows-ime\ListenerTypeIme.sln"
...
$dll = Join-Path $defaultOutputDirectory "ListenerTypeIme.dll"
...
$dll = Join-Path $outputDirectoryPath "ListenerTypeIme.dll"
...
if ($LASTEXITCODE -ne 0) {
  throw "ListenerTypeIme build failed with exit code $LASTEXITCODE"
}

if (-not (Test-Path $dll)) {
  throw "ListenerTypeIme.dll was not produced: $dll"
}
