...
APP="src-tauri/target/release/bundle/macos/Listener Type.app"
...
if [ "$INSTALL" = "1" ]; then
  ...
  pkill -f "Listener Type.app/Contents/MacOS/listener-type" 2>/dev/null || true
  sleep 1
  # 每次重装前重置 TCC：ad-hoc 签名 hash 每次构建都会变，旧授权立即失效，
  # 不重置就会出现"系统设置里看着已勾选实际不生效"。
  tccutil reset Accessibility com.listener.type 2>/dev/null || true
  tccutil reset Microphone com.listener.type 2>/dev/null || true
  rm -rf /Applications/Listener Type.app
  cp -R "$APP" /Applications/
  xattr -dr com.apple.quarantine /Applications/Listener Type.app 2>/dev/null || true
  echo "✓ 装好了：/Applications/Listener Type.app"
  echo "  打开方式：open /Applications/Listener Type.app"
fi
