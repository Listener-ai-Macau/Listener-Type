...
function Get-ManifestDllPath {
  ...
  try {
    ...
    Write-Host "[warn] Failed to read Listener Type IME registration manifest: $($_.Exception.Message)"
  }
...
function Get-LatestStagedDllPath {
  ...
  $dll = Get-ChildItem -Path $registrationRoot -Filter ListenerTypeIme.dll -Recurse -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -like "*\$folder\$Configuration\ListenerTypeIme.dll" } |
...
function Get-LegacyDllPath {
  ...
  return Join-Path $appRoot "windows-ime\$legacyFolder\$Configuration\ListenerTypeIme.dll"
}
...
if (-not (Test-IsAdministrator)) {
  throw "Unregistering the Listener Type TSF IME requires an elevated Administrator PowerShell."
}

foreach ($platform in @("x64", "Win32")) {
  $dll = Get-DllPath $platform
  if (-not (Test-Path $dll)) {
    Write-Host "[skip] ListenerTypeIme.dll not found ($platform): $dll"
    continue
  }
  ...
  Write-Host "[ok] Listener Type TSF IME unregistered ($platform)"
}
