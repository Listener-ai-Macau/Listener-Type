...
public static class ListenerTypeMicPrivacyWin32 {
...
function Send-KeyEdge($Vk, $KeyUp) {
  $flags = [ListenerTypeMicPrivacyWin32]::KEYEVENTF_EXTENDEDKEY
  if ($KeyUp) {
    $flags = $flags -bor [ListenerTypeMicPrivacyWin32]::KEYEVENTF_KEYUP
  }
  $scanCode = if ($Vk -eq 0xA3 -or $Vk -eq 0xA2) { 0x1D } else { 0 }
  [ListenerTypeMicPrivacyWin32]::keybd_event([byte]$Vk, [byte]$scanCode, $flags, [UIntPtr]::Zero)
}
...
function Focus-Window($Process) {
  ...
  [ListenerTypeMicPrivacyWin32]::ShowWindow($Process.MainWindowHandle, 9) | Out-Null
  [ListenerTypeMicPrivacyWin32]::SetForegroundWindow($Process.MainWindowHandle) | Out-Null
