...
### 短语已命中但整段声纹不匹配时必须进入分离验证（2026-09-05）

干扰者和主人同时说话时，整段候选的主人 embedding 可能被旁人污染。旧路径在
KWS/本地短语已经确认后，只要整段声纹不匹配就立即拒绝，目标声纹分离永远没有
机会运行；因此“干扰下偶尔能唤醒、偶尔完全无响应”会随时序变化。

现在只要已有明确短语证据但整段主人校验未通过，实时和终端边界都会把候选送入
同一个 `phrase_owner_mismatch` 分离恢复通道。分离波形仍必须独立同时通过完整短语
和已登记主人校验，不能用分离任务绕过任何拒绝门槛；若分离不可用或超出有界等待，
候选仍按原有拒绝策略结束。
