...
2026-06-11 按键口径说明：P13 执行时的硬件验证证据沿用了当时的 `KEY1`/EC11 录音键说法；现行产品方案已收敛为默认 `KEY3` 自定义键负责开始/停止录音，EC11 是开机键，开机后单击作为可配置自定义动作，双击保留蓝牙恢复。本文保留历史证据，但活跃验收和用户文档应使用 KEY3/自定义录音键口径。
...
| 8 | 端到端集成验证 | 固件 + Listener-Type | KEY3/自定义录音键 → BLE → ASR → 文本完整闭环 |
...
| 3.7 | 真实设备流式 smoke | 固件 + Listener-Type | HW | KEY3/自定义录音键触发录音期间 audio_data 持续送 ASR，停止后输出 final text | COM3/BLE 资源锁 | 后置；需要硬件锁，不和软件并行段抢资源 |
...
| 编号 | 场景 | 通过标准 |
|---|---|---|
| P13.1 | BLE host adapter smoke | KEY3/自定义录音键触发后软件端收到完整 session |
...
| S-BLE-4 | 真实录音键 recording smoke | 设备 KEY3/自定义录音键开始后 audio_data 持续送 ASR；停止后输出 final text |
...
- P13.3.7 真实录音键流式 smoke（历史记录称 KEY1/EC11，现行口径为 KEY3/自定义录音键）：BLE notify subscription 成功，录音键启动 embedded session，Volcengine ASR 返回 final text `这是一段新的合成语音，用来检查火山识别和蓝牙。`
...
- P13 当前无活跃阻塞项。
- `missing_packets=138` 是真实流式 smoke 中记录到的尾段 gap 注意事项；验收口径是 BLE notify 订阅、真实录音键 session（历史记录称 KEY1/EC11，现行口径为 KEY3/自定义录音键）、ASR final text 和 CLI 正常完成，均已满足。
