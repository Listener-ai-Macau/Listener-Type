...
- Installer registration hooks: `src-tauri/nsis/listener-type-ime-hooks.nsh`
...
- Dictation insertion can use the Windows IME path when the TSF service is registered and the target app accepts it.
- Installer hooks register and unregister both x64 and x86 TSF DLLs and clean up old registration on upgrade.
