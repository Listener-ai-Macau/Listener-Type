...
一句话：安装 Listener Type，配对名为 `listener` 的设备，按 EC11 旋钮说话，再按一次得到文字。

## 1. 安装软件

1.0.5 下载入口：`docs/quickstart/voice-keyboard-download.md`

- 推荐安装包：仓库根目录 `ListenerType_1.0.5_x64_en-US.msi`
- MSI SHA256：`3A3042F7B5B598DB0BD32D4EEC4F5B3F1B79919F41D0CED1530551C52DCD05BE`
- 配套固件：仓库根目录 `ListenerFirmware_1.0.5_ota.zip`
...
4. EC11 旋钮是开机键；开机后单击执行 EC11 自定义动作，默认开始或停止录音，旋转默认调节电脑音量，可在 Listener Type 设置里改成屏幕亮度或禁用。
5. KEY1-KEY4 和 EC11 单击是自定义动作键，默认 KEY3 发送 Ctrl+V。EC11 单击的底层入口是 `Shift+F13`；不要使用 F25 作为替代入口。
...
| 状态 | 含义 | 下一步 |
|---|---|---|
| Ready | 设备和软件都准备好 | 按 EC11 开始说话 |
| Recording | 正在录音 | 说完再按一次 EC11 |
