...
Listener 语音键盘是一个 BLE 桌面语音输入设备。它用自定义按键触发 Listener Type 动作，默认 EC11 单击用于开始/停止录音；EC11 旋钮也是开机键，开机后单击可在设备自定义键里继续调整，覆盖从安装、配对到第一次语音试玩的完整路径。
...
1.0.5 下载入口：`docs/quickstart/voice-keyboard-download.md`

- MSI：仓库根目录 `ListenerType_1.0.5_x64_en-US.msi`
- MSI SHA256：`3A3042F7B5B598DB0BD32D4EEC4F5B3F1B79919F41D0CED1530551C52DCD05BE`
- 固件 OTA：仓库根目录 `ListenerFirmware_1.0.5_ota.zip`
- OTA SHA256：`D993765AC07838F798B71863F87A69A59DAC9EE47CBBCE3315266FBBB631BADC`
...
| 操作 | 功能 |
|---|---|
| 按下 KEY3 | 默认发送 Ctrl+V，可在设备自定义键里调整 |
| 按下 EC11 旋钮 | 开机后执行 EC11 单击自定义动作，默认开始或停止语音录音 |
| 双击 EC11 旋钮 | 重置蓝牙配对并重新进入可配对状态 |
| 旋转 EC11 旋钮 | 默认调节电脑音量，可在 Listener Type 设置里改成屏幕亮度或禁用 |
| KEY1-KEY4 / EC11 单击 | 自定义动作键，未配置时只提供安全 fallback |
| 电脑键盘 `Esc` | 取消当前录音或处理中任务 |

默认 EC11 单击触发语音录音。KEY3 默认发送 Ctrl+V。

设备自定义键的 fallback 入口固定为：KEY1-KEY4 使用裸 F13-F24 手势组，EC11 单击使用 `Shift+F13` 作为专用入口。不要把 EC11 改成 F25；F25 不在当前 Windows/HID 热键支持矩阵内。
...
| 设备连上但录音没反应 | 确认 EC11 单击动作仍配置为开始/停止录音 |
...
| 状态 | 用户含义 | 恢复口径 |
|---|---|---|
| Ready | 可以开始录音 | 按 EC11 开始 |
| Recording | 正在听你说话 | 说完再按 EC11 停止 |
