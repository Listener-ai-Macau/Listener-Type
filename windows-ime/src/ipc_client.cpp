...
constexpr wchar_t kPipeNamePrefix[] = L"\\\\.\\pipe\\ListenerTypeImeSubmit";
...
ListenerTypePipeServer::ListenerTypePipeServer() = default;

ListenerTypePipeServer::~ListenerTypePipeServer() {
  Stop();
}

void ListenerTypePipeServer::Start(ListenerTypeTextService* service) {
  ...
  service_ = service;
  service_->AddRef();
  thread_ = std::thread(&ListenerTypePipeServer::Run, this);
}

void ListenerTypePipeServer::Stop() {
...
void ListenerTypePipeServer::Run() {
...
bool ListenerTypePipeServer::ReadJsonLine(HANDLE pipe, std::string* line) {
  line->clear();
  char buffer[1024] = {};
...
void ListenerTypePipeServer::HandleSubmitLine(HANDLE pipe, const std::string& line) {
...
bool ListenerTypePipeServer::WriteResult(HANDLE pipe,
...
void ListenerTypePipeServer::WakePipe() {
  if (pipe_name_.empty()) {
    return;
  }
  HANDLE pipe =
