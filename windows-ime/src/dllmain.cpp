...
STDAPI DllGetClassObject(REFCLSID clsid, REFIID iid, void** object) {
  ...
  *object = nullptr;
  ...
  if (clsid != CLSID_ListenerTypeTextService) {
    return CLASS_E_CLASSNOTAVAILABLE;
  }

  auto* factory = new (std::nothrow) ListenerTypeClassFactory();
  if (factory == nullptr) {
...
STDAPI DllRegisterServer() {
  return RegisterListenerTypeTextService(g_module);
}

STDAPI DllUnregisterServer() {
  return UnregisterListenerTypeTextService();
}
